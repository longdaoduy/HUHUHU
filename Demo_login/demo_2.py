                ...
                if not results:
                    ...
                    for dest in results:
                        with st.container(border=True): # Cập nhật container cho đẹp hơn
                            ...
                            st.write(f"  🗺️**Địa điểm:** {dest['location']}")
                            st.write(f"  🏷️**Thẻ:** {','.join(dest['tags'])}")
                            st.write(f" **Review:** {dest['review']}") # Dùng join cho đẹp
...
if 'authentication_status' not in st.session_state:
    st.session_state.authentication_status = None # Cần thiết cho stauth
...
if st.session_state.get('authentication_status') is True:
    ...
    with st.sidebar:
        ...
        st.header(f"👋 Xin chào, {st.session_state.name}!")
        st.header("🧭Điều hướng")
        
        # Thanh điều hướng (Selectbox)
        ...
        # Cập nhật state nếu lựa chọn thay đổi
        ...
        # Nút ĐĂNG XUẤT
        if authenticator:
            authenticator.logout('Đăng xuất','main') # Đặt nút Đăng xuất ở sidebar hoặc main, tùy bạn chọn
        
        st.markdown("----")
        st.markdown("🌐 **Website Sightseeing** © 2025 | Designed with ❤️ in Streamlit")
        st.markdown(
        """
        <style>
        ...
        <div class="top-bar">
            <span class = "top-bar-text"> VietNam UrbanQuest </span>
        </div>
        """,
        unsafe_allow_html=True)

    # Chạy hàm của trang được chọn
    PAGES[st.session_state.nav]()
    
# 2. Nếu người dùng CHƯA ĐĂNG NHẬP & YÊU CẦU ĐĂNG KÝ
elif st.session_state.get('show_signup') is True:
    login.screen_signupform()
    
    # Nút quay lại đăng nhập nếu người dùng đổi ý
    if st.button("⬅️ Quay lại Đăng nhập", key="back_to_login"):
        st.session_state.show_signup = False
        st.rerun()

elif st.session_state.get('authentication_status') in (None, False):
